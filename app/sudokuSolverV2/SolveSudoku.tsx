...
const SolveSudoku = (sudoku: number[][]): number[][] => {
    const solve = (sudoku: number[][]) => {
        const new_array = sudoku.flat()
        return solvePuzzle(new_array)
    };

    const puzzle = [...sudoku];
    const solution: number[] = solve(puzzle);

    const solvedSudoku: number[][] = Array(9).fill(Array(9).fill(null))

    for (let i = 0; i < solution.length; i++) {
        const row = Math.floor(i / 9)
        const col = i % 9
        solvedSudoku[row][col] = solution[i]
    }
