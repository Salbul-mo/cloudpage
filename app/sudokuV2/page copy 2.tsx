...
export default function Page() {
    ...
    const handleSolve = () => {
        ...
        try {
            ...
            // WASM 메모리에 보드 데이터 복사
            const boardPtr = solver._malloc(flatBoard.length);
            const resultPtr = solver._malloc(flatBoard.length);
            
            // 배열 데이터를 WASM 메모리에 복사
            for (let i = 0; i < flatBoard.length; i++) {
                solver.HEAP8[boardPtr + i] = flatBoard[i];
            }
            
            // WASM에서 제공하는 스도쿠 해결 함수 호출
            const success = solver._solve_sudoku(boardPtr, resultPtr); 
            
            if (success) {
                // 결과를 자바스크립트 배열로 다시 가져오기
                const solution = [];
                for (let i = 0; i < 9; i++) {
                    const row = [];
                    for (let j = 0; j < 9; j++) {
                        row.push(solver.HEAP8[resultPtr + i * 9 + j]);
                    }
                    solution.push(row);
                }
            ...
            // 메모리 해제
            solver._free(boardPtr);
            solver._free(resultPtr);
    ...
    const handleCheckUnique = () => {
        ...
        try {
            ...
            const hasUniqueSolution = solver._has_unique_solution(boardPtr, flatBoard.length);
            
            if (hasUniqueSolution === 1) {
                setMessage('이 퍼즐은 유일한 해결책이 있습니다.');
            } else if (hasUniqueSolution === 0) {
                setMessage('이 퍼즐은 여러 개의 해결책이 있거나 해결책이 없습니다.');
            } else {
                setMessage('유일성 검사 중 오류가 발생했습니다.');
            }
