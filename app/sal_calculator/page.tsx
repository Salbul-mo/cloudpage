...
const SalaryCalculator: React.FC = () => {
    ...
    const [minHourlyWage, setMinHourlyWage] = useState<number>(9860);
    const [mealAllowanceChecked, setMealAllowanceChecked] = useState<boolean>(false);
    const [carAllowanceChecked, setCarAllowanceChecked] = useState<boolean>(false);
    ...
    const handleInputChange = (
        ...
        if (typeof setter === typeof setNetSalary || typeof setter === typeof setTotalDeductions) {
            (setter as React.Dispatch<React.SetStateAction<string>>)(formatNumber(unformatted));
        } else {
            ...
            <div>
                <div className="space-y-2">
                    <div className="flex justify-between items-center bg-gray-50 rounded-t-lg p-3">
                        ...
                        <span className="text-lg text-indigo-600 font-bold">{formatNumber(result.hourlyWage)} 원</span>
                    </div>
                    <div className="flex justify-between items-center bg-gray-50 p-3">
                        <span className="font-medium text-gray-700">산출 연장근로</span>
                        <span className="text-lg text-indigo-600 font-bold">{result.overtimeHours} 시간</span>
                    </div>
                </div>
                <div className="mt-6 border-t pt-4">
                    ...
                    <div className="flex justify-between items-center p-3 border-b">
                        ...
                        <span className="font-medium">{formatNumber(result.basePay)} 원</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-b">
                        <span className="font-medium text-gray-700">연차수당 (주휴수당 대체)</span>
                        <span className="font-medium">{formatNumber(result.annualLeaveAllowance)} 원</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-b">
                        <span className="font-medium text-gray-700">연장수당</span>
                        <span className="font-medium">{formatNumber(result.overtimeAllowance)} 원</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-b">
                        <span className="font-medium text-gray-700">식대 (비과세)</span>
                        <span className="font-medium">{formatNumber(result.mealAllowance)} 원</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-b">
                        <span className="font-medium text-gray-700">차량유지비 (비과세)</span>
                        <span className="font-medium">{formatNumber(result.carAllowance)} 원</span>
                    </div>
                ...
                <div className="mt-6 border-t pt-4">
                    <div className="flex justify-between items-center p-3 border-b">
                        ...
                        <span className="text-xl font-bold">{formatNumber(result.totalGrossSalary)} 원</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-b">
                        <span className="text-xl font-medium text-gray-700">총 공제액</span>
                        <span className="text-xl font-bold text-red-600">- {formatNumber(result.totalDeductions)} 원</span>
                    </div>
                    <div className="flex justify-between items-center text-2xl font-extrabold bg-indigo-50 p-4 rounded-b-lg">
                        ...
                        <span className="text-indigo-800">{formatNumber(result.netSalary)} 원</span>
                    </div>
                </div>
            </div>
        );
